...
* **第一次握手丢失：**第一台计算机发送报文之后会启动一个定时器，在超时之后未收到服务器端的确认，会再次发送 SYN 请求，每次尝试的时间会是第一次的二倍，如果总的总尝试时间为75秒，此次建立链接失败。
* **第二次握手丢失：**在发送完 ACK+SYN 报文后会启动一个定时器，超时没有收到 ACK 确认，会再次发送，会进行多次重试。
  超时时间依旧每次翻倍，重试次数可设置。
* **第三次握手丢失：**服务端在发出 ACK+SYN 报文后会启动一个定时器，在超时触发还没收到 ACK 就确认是丢失了，会重试一次发送。
